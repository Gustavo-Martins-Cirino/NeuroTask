...
// Fallback replacements if the big regex above fails
...
// B) Insert .right-panel before Auth Overlay
...
`
if(!content.includes('<aside class="right-panel">')) {
  content = content.replace(/<!-- AUTH OVERLAY/, rightPanelHtml + '\n    <!-- AUTH OVERLAY');
}

// C) Move Nova Tarefa Button
const createBtnMatch = content.match(/<button class="create-btn"[^>]*>[\s\S]*?<\/button>/);
if(createBtnMatch && content.includes('<div class="nav-menu">')) {
   ...
   if(navMenuEnd !== -1) {
     ...
     // fallback
     content = content.replace(/<\/div>\s*<\/div>\s*<!-- MAIN CONTENT -->/, '\n        ' + createBtnMatch[0] + '\n      </div>\n    </div>\n\n    <!-- MAIN CONTENT -->');
   }
}
...
`
if(!content.includes('window.toggleZenChat')) {
  // If there's a script tag at the end, replace within it, otherwise add it before </body>
