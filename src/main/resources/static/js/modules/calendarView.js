/**
 * Calendar View - Renderiza calendário com time-blocking (8h-20h)
 *
 * Responsável por:
 * - Criar grid de horários (slots de 1 hora)
 * - Renderizar tarefas agendadas nos slots corretos
 * - Integrar com dragController para drop zones
 */
...
export const calendarView = {
  /**
   * Renderiza agenda 24h fit-to-screen, slots flexíveis
   */
  render(tasks, containerSelector) {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      slot.addEventListener("drop", async (e) => {
        e.preventDefault();
        slot.classList.remove("drag-over");
        // Cálculo da hora baseada na posição Y do mouse
        const rect = slot.getBoundingClientRect();
        const offsetY = e.clientY - rect.top;
        const totalHeight = rect.height;
        // Preferência: horários cheios ou múltiplos de 15
        let hour = Math.floor((offsetY / totalHeight) * 24);
        let minute = 0;
        // Se preferir múltiplos de 15:
        // minute = Math.round((((offsetY / totalHeight) * 24) % 1) * 4) * 15;
        ...
        if (habitId) {
          // Cria tarefa a partir do hábito
      ...
      // Tarefas agendadas neste horário
      safeTasks
        .filter((t) => {
